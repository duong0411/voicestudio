...
public class SysParamsServiceImpl extends BaseServiceImpl<SysParamsDao, SysParamsEntity> implements SysParamsService {
    ...
    public void initServerSecret() {
        ...
        if (StringUtils.isBlank(secretParam) || "null".equals(secretParam)) {
            ...
            upsertParamValue(Constant.SERVER_SECRET, newSecret, "string", "服务器密钥");
        }

        // 初始化SM2密钥对
        initSM2KeyPair();

        // Sync Google OAuth configurations from environment variables if present
        String envClientId = System.getenv("CLIENT_ID");
        String envClientSecret = System.getenv("CLIENT_SECRET");
        if (StringUtils.isNotBlank(envClientId)) {
            upsertParamValue("google.clientId", envClientId, "string", "Google OAuth Client ID");
        }
        if (StringUtils.isNotBlank(envClientSecret)) {
            upsertParamValue("google.clientSecret", envClientSecret, "string", "Google OAuth Client Secret");
        }
    }
    ...
    private void initSM2KeyPair() {
        ...
        if (StringUtils.isBlank(publicKey) || StringUtils.isBlank(privateKey) ||
            ...
            // 使用upsert更新数据库中的密钥对（行不存在时自动INSERT）
            upsertParamValue(Constant.SM2_PUBLIC_KEY, newPublicKey, "string", "SM2公钥（自动生成）");
            upsertParamValue(Constant.SM2_PRIVATE_KEY, newPrivateKey, "string", "SM2私钥（自动生成）");
        }
    }

    /**
     * 插入或更新参数值（upsert）：行存在则UPDATE，不存在则INSERT
     */
    private void upsertParamValue(String paramCode, String paramValue, String valueType, String remark) {
        String existing = getValue(paramCode, false);
        if (existing == null) {
            ...
            updateValueByCode(paramCode, paramValue);
        }
    }
