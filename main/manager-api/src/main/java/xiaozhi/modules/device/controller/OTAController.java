...
public class OTAController {
    ...
    @Operation(summary = "OTA")
    @PostMapping
    public ResponseEntity<String> checkOTAVersion(
            @RequestBody DeviceReportReqDTO deviceReportReqDTO,
            @Parameter(name = "Device-Id", description = "", required = true, in = ParameterIn.HEADER) @RequestHeader("Device-Id") String deviceId,
            @Parameter(name = "Client-Id", description = "", required = false, in = ParameterIn.HEADER) @RequestHeader(value = "Client-Id", required = false) String clientId) {
    ...
    public ResponseEntity<String> getOTA() {
        ...
        if (StringUtils.isBlank(mqttUdpConfig)) {
            return ResponseEntity.ok("OTA，mqtt_gateway，，【server.mqtt_gateway】");
        }
        String wsUrl = sysParamsService.getValue(Constant.SERVER_WEBSOCKET, true);
        if (StringUtils.isBlank(wsUrl) || wsUrl.equals("null")) {
            return ResponseEntity.ok("OTA，websocket，，【server.websocket】");
        }
        String otaUrl = sysParamsService.getValue(Constant.SERVER_OTA, true);
        if (StringUtils.isBlank(otaUrl) || otaUrl.equals("null")) {
            return ResponseEntity.ok("OTA，ota，，【server.ota】");
        }
        return ResponseEntity.ok("OTA，websocket：" + wsUrl.split(";").length);
    }
