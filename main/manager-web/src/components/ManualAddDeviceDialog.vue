...
export default {
  ...
  watch: {
    'deviceForm.board'(newVal) {
      if (newVal === 'web-client') {
        if (!this.deviceForm.macAddress) {
          this.deviceForm.macAddress = this.generateRandomMac();
        }
        if (!this.deviceForm.appVersion) {
          this.deviceForm.appVersion = '1.0.0';
        }
      }
    }
  }
