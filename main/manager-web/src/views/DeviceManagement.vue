            ...
            <el-table ref="deviceTable" :data="paginatedDeviceList" class="transparent-table"
              ...
              <el-table-column :label="$t('device.operation')" align="center">
                <template slot-scope="scope">
                  ...
                  <el-button v-if="scope.row.model === 'web-client'" size="mini" type="text" @click="handleOpenWebClient(scope.row)">
                    {{ $t('device.openWebClient') }}
                  </el-button>
...
export default {
  ...
  methods: {
    ...
    handleOpenWebClient(row) {
      const hostname = window.location.hostname;
      const otaUrl = `${window.location.protocol}//${hostname}:8002/xiaozhi/ota/`;
      const url = `http://${hostname}:8006/index.html?mac=${encodeURIComponent(row.macAddress)}&board=web-client&otaUrl=${encodeURIComponent(otaUrl)}&connect=true`;
