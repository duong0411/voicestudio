...
class StreamTTSPerformanceTester:
                ...
                async with websockets.connect(ws_url, extra_headers={"X-NLS-Token": token}) as ws:
                    ...
                    while True:
                        ...
                        if isinstance(response, bytes):
                            ...
                            print(f"[TTS] {i+1} : {latency:.3f}s")
                            break
                        elif isinstance(response, str):
                            data = json.loads(response)
                            if data["header"]["name"] == "TaskFailed":
                                raise Exception(f": {data['payload']['error_info']}")

            except Exception as e:
                print(f"[TTS] {i+1}: {str(e)}")
                    ...
                    while not task_started:
                        ...
                        if isinstance(msg, str):
                            ...
                            if event == "task-started":
                                ...
                                raise Exception(f": {header.get('error_message', 'Unknown error')}")
        ...
        for i in range(test_count):
            try:
                ...
                async with websockets.connect(tts_url) as ws:
                    ...
                    latency = time.time() - start_time
                    latencies.append(latency)
                    print(f"[PaddleSpeechTTS] {i+1} : {latency:.3f}s")
                ...
                print(f"[PaddleSpeechTTS] {i+1}: {str(e)}")
                latencies.append(None)
    ...
    async def test_indexstream_tts(self, text=None, test_count=5):
        ...
        for i in range(test_count):
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.post(api_url, json=payload, timeout=10) as resp:
                        ...
                        async for chunk in resp.content.iter_any():
                            ...
                            latency = time.time() - start_time
                            latencies.append(latency)
                            print(f"[IndexStreamTTS] {i+1} : {latency:.3f}s")
                            resp.close()
                            break
                        else:
                            latencies.append(None)
                ...
                print(f"[IndexStreamTTS] {i+1}: {str(e)}")
                latencies.append(None)
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.get(api_url, params=params, headers=headers, timeout=10) as resp:
                        ...
                        async for _ in resp.content.iter_any():
                            latency = time.time() - start_time
                            latencies.append(latency)
                            print(f"[LinkeraiTTS] {i+1} : {latency:.3f}s")
                            break
                        else:
                            latencies.append(None)

            except Exception as e:
                print(f"[LinkeraiTTS] {i+1}: {str(e)}")
                latencies.append(None)
    ...
    def _print_results(self, test_text, test_count):
        ...
        print(tabulate(table_data, headers=["TTS", "()", ""], tablefmt="grid"))
        print("\n：（、、），")
...
async def main():
    ...
    parser.add_argument("--count", type=int, default=5, help="TTS")
