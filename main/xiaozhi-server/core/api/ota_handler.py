...
class OTAHandler(BaseHandler):
    ...
    def generate_password_signature(self, content: str, secret_key: str) -> str:
        ...
        try:
            ...
            self.logger.bind(tag=TAG).error(f"MQTT: {e}")
            return ""
    ...
    async def handle_post(self, request):
        ...
        try:
            ...
            self.logger.bind(tag=TAG).debug(f"OTA: {request.method}")
            self.logger.bind(tag=TAG).debug(f"OTA: {request.headers}")
            self.logger.bind(tag=TAG).debug(f"OTA: {data}")

            device_id = request.headers.get("device-id", "")
            if device_id:
                self.logger.bind(tag=TAG).info(f"OTAID: {device_id}")
            else:
                raise Exception("OTAID")

            client_id = request.headers.get("client-id", "")
            if client_id:
                self.logger.bind(tag=TAG).info(f"OTAClientID: {client_id}")
            else:
                raise Exception("OTAClientID")
            ...
            if mqtt_gateway_endpoint:
                ...
                if signature_key:
                    ...
                    self.logger.bind(tag=TAG).warning("MQTT，")
                ...
                self.logger.bind(tag=TAG).info(f" {device_id} MQTT")
            ...
            try:
                ...
                if chosen_url:
                    ...
                    self.logger.bind(tag=TAG).info(
                        f" {device_id}  {chosen_version} [，server.vision_explain]-> {chosen_url} "
                    )
                else:
                    self.logger.bind(tag=TAG).info(
                        f" {device_id} : {device_version}"
                    )
            ...
            self.logger.bind(tag=TAG).error(f"OTA POST: {e}")
    ...
    async def handle_get(self, request):
        ...
        try:
            ...
            message = f"OTA，websocket：{websocket_url}"
            response = web.Response(text=message, content_type="text/plain")
        except Exception as e:
            self.logger.bind(tag=TAG).error(f"OTA GET: {e}")
            response = web.Response(text="OTA", content_type="text/plain")
        finally:
            self._add_cors_headers(response)
            return response
