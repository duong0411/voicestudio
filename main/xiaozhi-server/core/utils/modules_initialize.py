...
def initialize_modules(
    ...
    if init_tts:
        ...
        logger.bind(tag=TAG).info(f": tts {select_tts_module}")
    ...
    if init_llm:
        ...
        logger.bind(tag=TAG).info(f": llm {select_llm_module}")
    ...
    if init_intent:
        ...
        logger.bind(tag=TAG).info(f": intent {select_intent_module}")
    ...
    if init_memory:
        ...
        logger.bind(tag=TAG).info(f": memory {select_memory_module}")
    ...
    if init_vad:
        ...
        logger.bind(tag=TAG).info(f": vad {select_vad_module}")


    if init_asr:
        select_asr_module = config["selected_module"]["ASR"]
        modules["asr"] = initialize_asr(config)
        logger.bind(tag=TAG).info(f": asr {select_asr_module}")
    return modules
...
def initialize_voiceprint(asr_instance, config):
    ...
    try:
        ...
        logger.bind(tag=TAG).info("ASR")
        logger.bind(tag=TAG).info(f": {len(voiceprint_config['speakers'])}")
        return True
