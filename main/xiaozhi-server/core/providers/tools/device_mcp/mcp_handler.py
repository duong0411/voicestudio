...
async def send_mcp_message(conn: "ConnectionHandler", payload: dict):
    ...
    if not conn.features.get("mcp"):
        logger.bind(tag=TAG).warning("MCP，MCP")
        return
    ...
    try:
        ...
        logger.bind(tag=TAG).debug(f"MCP: {message}")
...
async def handle_mcp_message(
    ...
    """MCP,、"""
    logger.bind(tag=TAG).debug(f"MCP: {str(payload)[:100]}")

    if not isinstance(payload, dict):
        logger.bind(tag=TAG).error("MCPpayload")
        return
        ...
        if msg_id == 1:  # mcpInitializeID
            ...
            if isinstance(result, dict) and "tools" in result:
                ...
                for i, tool in enumerate(tools_data):
                    ...
                    logger.bind(tag=TAG).debug(f" #{i+1}: {name}")
                ...
                if next_cursor:
                    logger.bind(tag=TAG).debug(f"，nextCursor: {next_cursor}")
...
async def call_mcp_tool(
    ...
    logger.bind(tag=TAG).info(f"mcp: {actual_name}，: {args}")
    await send_mcp_message(conn, payload)
