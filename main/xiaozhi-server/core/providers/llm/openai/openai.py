...
class LLMProvider(LLMProviderBase):
    def __init__(self, config):
        ...
        logger.debug(
            f": {self.temperature}, {self.max_tokens}, {self.top_p}, {self.frequency_penalty}"
        )
    ...
    def _apply_thinking_disabled(self, request_params: dict):
        ...
        for disabled_domain, params in THINKING_DISABLED_DOMAINS.items():
            if disabled_domain in domain:
                ...
                logger.bind(tag=TAG).info(f" {domain} ，: {params}")
                break
            ...
            for chunk in stream:
                if getattr(chunk, "choices", None):
                    ...
                    logger.bind(tag=TAG).info(
                        f"Token ： {getattr(usage_info, 'prompt_tokens', '')}，"
                        f" {getattr(usage_info, 'completion_tokens', '')}，"
                        f" {getattr(usage_info, 'total_tokens', '')}"
                    )
        finally:
            stream.close()
