  ...
  try {
    ...
    for (let i = 0; i < bits.length; i++) {
      ...
      for (let j = 0; j < samplesPerBit; j++) {
        ...
        floatBuf[i * samplesPerBit + j] = Math.sin(2 * Math.PI * freq * t) * 0.5
      }
    }
    ...
    console.log(t('deviceConfig.audioGenerationSuccess') + '，:', bits.length, t('deviceConfig.samplePoints') + ':', floatBuf.length)
  ...
  try {
    ...
    setTimeout(() => {
      if (audioContext.value) {
        console.log('，src:', audioFilePath.value.length)
