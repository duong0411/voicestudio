...
async function checkESP32Connection() {
  ...
  try {
    ...
    console.log(`${t('deviceConfig.xiaozhi')}:`, isConnectedToESP32.value)
  }
  catch (error) {
    isConnectedToESP32.value = false
    emit('connection-status', false)
    console.log('xiaozhi:', error)
  }
  finally {
  ...
  try {
    ...
    if (response.statusCode === 200 && response.data) {
      ...
      if (data.success && Array.isArray(data.networks)) {
        ...
        console.log(`${t('deviceConfig.scanSuccess')}， ${data.aps.length} ${t('deviceConfig.networks')}`)
      }
