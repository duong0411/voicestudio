...
import { getTranslation } from './localization.js?v=0205';
import { getMicCaptureMode } from './source-selector.js?v=0205';
...
class UIController {
    ...
    init() {
        ...
        // Auto connect if query parameter is present
        ...
        if (dialBtn) {
            dialBtn.addEventListener('click', () => {
                ...
                if (isConnected) {
                    ...
                    this.addChatMessage(getMsg('status.disconnectedGoodbye'), false);
                } else {
                    ...
                    if (!otaUrlInput || !otaUrlInput.value.trim()) {
                        ...
                        this.addChatMessage(getMsg('status.fillOtaUrl'), false);
                        return;
                    }
                        ...
                        if (typeof window.startCamera === 'function') {
                            window.startCamera().then(success => {
                                if (success) {
                                    ...
                                    cameraBtn.querySelector('.btn-text').textContent = getMsg('buttons.camera');
                                } else {
                                    this.addChatMessage(getMsg('status.micUnavailable'), false);
                                }
                            }).catch(error => {
                                log(`: ${error.message}`, 'error');
                            });
                        } else {
                            log('startCamera', 'warning');
                        }
                    }
                }, 300);
            });
            ...
            recordBtn.addEventListener('click', () => {
                ...
                recordTimer = setTimeout(() => {
                    ...
                    if (audioRecorder.isRecording) {
                        ...
                        recordBtn.querySelector('.btn-text').textContent = '';
                    } else {
    ...
    updateDialButton(isConnected) {
        ...
        if (dialBtn) {
            if (isConnected) {
                ...
                dialBtn.querySelector('.btn-text').textContent = getMsg('buttons.hangUp');
                ...
                dialBtn.querySelector('.btn-text').textContent = getMsg('buttons.dial');
        ...
        if (recordBtn) {
            recordBtn.querySelector('.btn-text').textContent = isConnected ? getMsg('buttons.record') : '';
        }
        ...
        if (recordBtn) {
            ...
            const hideBrowserMic = window.__HIDE_BROWSER_MIC === true;
            if (isConnected && microphoneAvailable && !hideBrowserMic) {
                ...
                if (!microphoneAvailable) {
                    recordBtn.title = window.isHttpNonLocalhost ? 'http，，' : '';
                } else if (hideBrowserMic) {
                    recordBtn.title = 'ESP32';
                } else {
                    recordBtn.title = '';
                }
                // Restore record button to normal state
    ...
    updateMicrophoneAvailability(isAvailable, isHttpNonLocalhost) {
        ...
        if (!isAvailable) {
            ...
            recordBtn.querySelector('.btn-text').textContent = '';
            recordBtn.title = isHttpNonLocalhost ? 'http，，' : '';
    ...
    switchLive2DModel() {
        ...
        if (app && app.live2dManager) {
            app.live2dManager.switchModel(selectedModel)
                .then(success => {
                    if (success) {
                        this.addChatMessage(`${getMsg('status.modelLoaded')}${selectedModel}`, false);
                    } else {
                        this.addChatMessage(getMsg('status.modelLoadFailed'), false);
                    }
                })
                .catch(error => {
                    console.error(':', error);
            ...
            this.addChatMessage(getMsg('status.live2dNotInit'), false);
        }
    }
    ...
    async handleApplyWakeword() {
        ...
        try {
            ...
            this.addChatMessage(`: ${error.message}`, false);
        } finally {
            if (applyWakewordBtn) {
    ...
    startAIChatSession() {
        this.addChatMessage(getMsg('status.connectedWelcome'), false);
        if (window.__HIDE_BROWSER_MIC) {
            this.addChatMessage(getMsg('status.esp32AudioOnly'), false);
            return;
        }
        // Check microphone availability and show error messages if needed
        if (!window.microphoneAvailable) {
            if (window.isHttpNonLocalhost) {
                this.addChatMessage(getMsg('status.micBlocked'), false);
            } else {
                this.addChatMessage(getMsg('status.micWarning'), false);
            }
        }
        // Start recording only if microphone is available (continuous mode: auto-start stream)
        if (window.microphoneAvailable && getMicCaptureMode() === 'continuous') {
            const recordBtn = document.getElementById('recordBtn');
            if (recordBtn) {
                recordBtn.click();
            }
    ...
    async handleConnect() {
        ...
        try {
            ...
            if (!otaUrlInput || !otaUrlInput.value) {
                this.addChatMessage(getMsg('status.otaFailed'), false);
                return;
            }
            ...
            if (dialBtn) {
                dialBtn.classList.add('dial-active');
                dialBtn.querySelector('.btn-text').textContent = getMsg('status.connecting');
                dialBtn.disabled = true;
            }

            // Show connecting message
            this.addChatMessage(getMsg('status.connecting'), false);
            ...
            if (isConnected) {
                ...
                throw new Error(getMsg('status.otaFailed'));
            }
        } catch (error) {
            ...
            if (dialBtn) {
                ...
                dialBtn.querySelector('.btn-text').textContent = '';
                dialBtn.classList.remove('dial-active');
    ...
    async triggerWakewordDial(wakeWord = '') {
        const wsHandler = getWebSocketHandler();
        const now = Date.now();
        ...
        this.addChatMessage(getMsg('status.wakeWordDetected').replace('{wakeWord}', wakeWord), false);
    ...
    addMCPTool() {
        ...
        toolDiv.innerHTML = `
            <div class="property-item">
                ...
                <input type="text" placeholder="" value="">
                <button class="remove-property" onclick="uiController.removeMCPTool('${toolId}')"></button>
            </div>
        `;
