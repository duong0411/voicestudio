...
/**
 * @typedef {'push-to-talk' | 'continuous'} MicCaptureMode
 */
...
export class AudioRecorder {
    constructor() {
        ...
        /** Called after final Opus/end frame is sent so the client can send listen/stop */
        this.onAfterAudioFlush = null;
        /** Called when recording starts, after WS is ready — send listen/start for each PTT session */
        this.onBeforeAudioSend = null;
        this.onVisualizerUpdate = null;
    }

    /**
     * @param {MicCaptureMode} mode
     */
    setCaptureMode(mode) {
        this.captureMode = mode === 'continuous' ? 'continuous' : 'push-to-talk';
    }
    ...
    async createAudioProcessor() {
        ...
        try {
            if (this.audioContext.audioWorklet) {
                ...
                log('AudioWorklet，ScriptProcessorNode', 'warning');
    ...
    createScriptProcessor() {
        try {
            ...
            silent.connect(this.audioContext.destination);
            log('ScriptProcessorNode', 'warning');
            return { node: scriptProcessor, type: 'processor' };
        } catch (fallbackError) {
            log(`: ${fallbackError.message}`, 'error');
            return null;
        }
    }
    ...
    async start() {
        ...
        try {
            ...
            if (this.websocket && this.websocket.readyState === WebSocket.OPEN) {
                if (this.onBeforeAudioSend) {
                    try {
                        this.onBeforeAudioSend();
                    } catch (e) {
                        log(`onBeforeAudioSend: ${e.message}`, 'error');
                    }
                }
                log(``, 'info');
            } else {
    ...
    stop() {
        ...
        try {
            ...
            if (this.onAfterAudioFlush) {
                try {
                    this.onAfterAudioFlush();
                } catch (e) {
                    log(`onAfterAudioFlush: ${e.message}`, 'error');
                }
            }
            log('PCM', 'success');
            return true;
